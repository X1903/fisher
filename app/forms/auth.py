...
class RegisterForm(Form):
    ...
    password = PasswordField(validators=[DataRequired(message='密码不可以为空, 请输入你的密码'), Length(6, 32, message='密码为6-32个字符')])
    ...
    def validate_email(self, field):
        # db.session
        if User.query.filter_by(email=field.data).first():   # first()不管查询到多少条,只返回一条
